 ...
 * 验证码注册用户
 */
export const registerCode = (userInfo: object) => {
	return request({
		url: '/admin/register/user/code',
		method: 'post',
		data: userInfo,
	});
};
...
export function validateEmail(rule: any, value: any, callback: any, isEdit: boolean) {
	if (isEdit) {
		return callback();
	}
	getDetails({ email: value }).then((response) => {
		const result = response.data;
		if (result !== null) {
