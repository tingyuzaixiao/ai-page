...
const other = {
	...
	useUrlParser: (url: string) => {
		return useUrlParser(url);
	},
};
...
const useUrlParser = (targetUrl: string) => {
	const protocol = ref('');
	const host = ref('');
	const port = ref('');
	const uri = ref(''); // 端口后的路径部分（如 /detect）
	const isIP = ref(false);
	const queryParams = ref({});

	try {
		const parsedUrl = new URL(targetUrl);
		protocol.value = parsedUrl.protocol.replace(':', '');
		host.value = parsedUrl.hostname;
		port.value = parsedUrl.port || (protocol.value === 'https' ? '443' : '80');
		uri.value = parsedUrl.pathname; // 关键：提取路径（如 "/detect"）
		...
		isIP.value = ipPattern.test(host.value);
	} catch (e) {
		console.error('URL 解析失败:', e);
	}
	return { protocol, host, port, uri, isIP, queryParams };
};
