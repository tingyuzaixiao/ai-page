...
export const rule = {
	/**
	 * 校验 数据集版本名称是否符合规范 例如 V1.0：V + 版本号
	 * 名称校验
	 */
	validatorDatasetVersion(rule: any, value: any, callback: any) {
		//限制字符串格式为 V+数字+.+数字
		const acount = /^V\d{1,2}\.\d{1,2}$/;
		if (value && !acount.test(value)) {
			callback(new Error('请输入正确版本号 例如 V1.0：V + 版本号'));
		} else {
			callback();
		}
	},
