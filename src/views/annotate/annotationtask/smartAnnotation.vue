                  ...
                  <template #default="scope">
                    ...
                    <div v-else>
                      ...
                      <el-button icon="EditPen"
                        @click="datasetAnnotation(scope.row.taskId, props.row.baseDatasetName, scope.row.labelType, scope.row.subLabelType)"
                        ...
                        <el-dropdown>
                          ...
                          <template #dropdown>
                            <el-dropdown-menu>
                              ...
                              <!-- <el-dropdown-item @click="openImportReferenceLabelDialog(scope.row)">
                                {{ t('annotationTask.importReferenceLabel') }}
                              </el-dropdown-item> -->
                            </el-dropdown-menu>
                          </template>
                        </el-dropdown>
...
<script lang="ts" name="smartAnnotationTask" setup>
import { BasicTableProps, useTable } from '/@/hooks/table';
import { delIntelligentObj, fetchIntelligentList } from '/@/api/annotate/labelIntelligent';
...
const { sub_label_detect_type, sub_label_seg_type } = useDict('sub_label_detect_type', 'sub_label_seg_type');
//切换场景 0--模型展示， 1--创建模型
const scene = ref(0);
...
//基准数据集标注
const datasetAnnotation = (taskId: string, baseDatasetName: string, labelType: string, subLabelType?: string) => {
  ...
  router.push({ path, query: { taskId, baseDatasetName, subLabelType } });
};

const exportLabelTask = (type: string, row: any) => {
  //判断导出类型，是json还是csv
  const params = { taskId: row.taskId, baseDatasetId: row.baseDatasetId, labelType: row.labelType, subLabelType: row.subLabelType };
