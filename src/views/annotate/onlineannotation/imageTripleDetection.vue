    ...
    <div class="layout-padding-auto layout-padding-view" v-loading="handelLoading" ref="mainDiv"
        ...
        <div v-show="countMode" class="flex gap-10 w-full items-center justify-between">
          ...
          <div class="flex gap-2" style="flex:1 1 auto; justify-content: flex-end;">
            ...
            <template v-for="tool in toolList" :key="tool.type">
              <el-tooltip v-if="tool.subLabelType === '0'" class="item" effect="light"
      ...
      <div v-show="countMode" class="w-full h-full flex overflow-y-auto" v-loading="loading">
        ...
        <div v-loading="state.loading"
          ...
          <div class="left-top" style="display: flex; flex-direction: row;">

            <div class="left-top-left"
              style="flex: 1 1 50%; min-width: 0; min-height: 0; display: flex; flex-direction: column; height: 100%;">

              <!-- 上半部分 canvas -->
              <div class="canvas-wrapper" style="flex: 1; position: relative; min-height: 0; margin-bottom:5px">
                <span class="corner-label">Positive</span>
                <div class="corner-control">
                  <el-checkbox v-model="positiveStatus">有误</el-checkbox>
                </div>
                <canvas ref="positiveRef" class="positive-container"
                  style="width: 100%; height: 100%; max-width: 100%; max-height: 100%; display: block; background-color: #F7F7F7;"></canvas>
              </div>

              <!-- 下半部分 canvas -->
              <div class="canvas-wrapper" style="flex: 1; position: relative; min-height: 0;">
                <span class="corner-label">Negative</span>
                <div class="corner-control">
                  <el-checkbox v-model="negativeStatus">有误</el-checkbox>
                </div>
                <canvas ref="negativeRef" class="negative-container"
                  style="width: 100%; height: 100%; max-width: 100%; max-height: 100%; display: block; background-color: #F7F7F7;"></canvas>
              </div>

            </div>
            <!-- 右侧部分，宽度占 50% -->
            <div ref="leftTopDiv" class="left-top-right"
              style="flex: 1 1 50%; min-width: 0; min-height: 0; padding-left: 5px; display: flex; flex-direction: column; position: relative; overflow: hidden;">

              <!-- 上部分内容 -->
              <!-- <FileDirTree ref="fileDirTreeRef" :fileTreeData="fileTreeData" @node-click="handleNodeClick"
                :placeholder="'请输入文件目录名称'" :baseDatasetName="'triplets_shard_0000' as string" /> -->


              <div style="position: relative; flex: 1; width: 100%; min-height: 0;">
                <canvas ref="imageCanvasRef" class="bgimage"
                  style="width: 100%; height: 100%; max-width: 100%; max-height: 100%; display: block;"></canvas>
                <canvas ref="canvasRef" class="canvas-container"
                  style="width: 100%; height: 100%; max-width: 100%; max-height: 100%; display: block;"></canvas>
                <canvas ref="tempCanvasRef" class="tempcanvas"
                  style="width: 100%; height: 100%; max-width: 100%; max-height: 100%; display: block;"></canvas>
                <!-- 右键菜单 -->
                <div v-if="menuVisible" :style="{ top: menuPosition.y + 'px', left: menuPosition.x + 'px' }"
                  ...
                  <ul v-show="filteredItems().length !== 0" class="menu-list">
                    ...
                    <li v-for="(item, index) in filteredItems()" :key="index"
                      @click="menuVisible = false; menuUuid = ''; changeCheck(-1, item.tagId)"
              ...
              <div class="pathMesg" v-show="showPathMesg && switchPathMesg">
                ...
                <div class="mb-2">
                  <el-tooltip :content="pathTagName" placement="top" effect="light"
                    ...
                    <div v-overflow="handleOverflow(pathTagName)"
                ...
                <div class="mb-2">
                  <span>备注：</span>
                  <el-mention v-model="pathRemark" type="textarea" style="width: 150px" @input="havenRemark = true;" />
                </div>
                <div class="tool mt-2">
              ...
              <div v-if="imageList.length" class="flex gap-3" style="flex-wrap: nowrap; height: 100%;">
                <div v-for="(image, index) in imageList" :key="image.fileId" :ref="el => setImageRef(el, index)"
                  ...
                  <div
                    ...
                    <el-image class="w-full h-32" style="z-index: 2;" :src="formatUrl(image.bucketName, image.fileName)"
                      :zoom-rate="1.2" :max-scale="7" :min-scale="0.2" :initial-index="index" fit="contain"
                    ...
                    <el-checkbox :key="image.fileId" :label="image.fileName" :value="image.fileId"
                      ...
                      <div @click="!positionList.is_show_mask ? gotoImageFromMutil(index) : null"
                        ...
                        <el-image v-show="imageVisibility[index]" class="w-full h-40 object-contain" style="z-index: 2;"
                          :src="currentFileUrl + image.bucketName + '/' + image.fileName" fit="contain" :zoom-rate="1.2"
...
<script setup lang="ts" name="tripletDetection">
...
const ShortCutKey = defineAsyncComponent(() => import('../onlineannotation/shortCutKey.vue'));
const FileDirTree = defineAsyncComponent(() => import('../onlineannotation/fileDirTree.vue'));
const TabBarArea = defineAsyncComponent(() => import('../onlineannotation/tabBarArea.vue'));
const ObjectBarArea = defineAsyncComponent(() => import('../onlineannotation/objectBarArea.vue'));
...
const filterbyBucketName = async (pageNum: number = 1, pageSize: number = 18, changePage: boolean = true) => {
  ...
  try {
    ...
    getLabeledTotal({ taskId: "1954721123684265988", bucketName: state.queryForm.bucketName }).then((res) => {
...
let instance_positive: CanvasSelect;
let instance_negative: CanvasSelect;
...
const imagePositiveList: Ref<imageItem[]> = ref([]); //保存已滚动加载的图片
const imageNegativeList: Ref<imageItem[]> = ref([]); //保存已滚动加载的图片
...
const formatUrl = (bucketName: string, fileName: string, noUrl: boolean = false) => {
...
const showImage = (index: number, fileId: string, bucketName: string, fileName: string, isSaved: boolean) => {
  if (isSaved) {
    ...
    instance_temp.setImage(formatUrl(bucketName, fileName));
...
const gotoImageFromMutil = async (index: number) => {
  ...
  // }
  state.queryForm.bucketName = 'dataset/' + "triplets_shard_0000" + '/base/images/dataset/anchor';
...
const getPositiveList = async (pageNum: number = 1, pageSize: number = 18, changePage: boolean = true) => {
  loading.value = true;
  try {
    const response = await fetchBaseImageListNew({
      baseDatasetId: state.queryForm.baseDatasetId,
      bucketName: 'dataset/triplets_shard_0000/base/images/dataset/positive',
      current: pageNum,
      size: pageSize,
      fileName: '',
      isLabeled: state.queryForm.isLabeled,
    ...
    const data = response.data;
    const { records } = data;
    imagePositiveList.value.length = 0;
    imagePositiveList.value.push(...records);
  } catch (error) {
    console.error('Failed to load images', error);
  } finally {
  }
...
const getNegativeList = async (pageNum: number = 1, pageSize: number = 18, changePage: boolean = true) => {
  loading.value = true;
  try {
    const response = await fetchBaseImageListNew({
      baseDatasetId: state.queryForm.baseDatasetId,
      bucketName: 'dataset/triplets_shard_0000/base/images/dataset/negative',
      current: pageNum,
      size: pageSize,
      fileName: '',
      isLabeled: state.queryForm.isLabeled,
    ...
    const data = response.data;
    const { records } = data;
    imageNegativeList.value.length = 0;
    imageNegativeList.value.push(...records);
  } catch (error) {
    console.error('Failed to load images', error);
  } finally {
  }
...
const gotoImage = async (bucketName: string, fileName: string, nextfileId: string, index: number, reset: boolean = false) => {
  ...
  if (isSaved(nextfileId) || selectItem.length === 0) {
    ...
    const tool = toolList.find(item => item.subLabelType === "0");
    drawShapes(tool?.type ?? 1, false);
  }
  // 更换选中项、图片
  selectedList.value = [];
  console.log("***************", bucketName);
  const negativeImage = imageNegativeList.value[index];
  const positiveImage = imagePositiveList.value[index];
  instance_bgimg.setImage(formatUrl(bucketName, fileName));
  instance.setImage(formatUrl(bucketName, fileName), 0);
  instance_temp.setImage(formatUrl(bucketName, fileName));
  instance_positive.setImage(formatUrl(positiveImage.bucketName, positiveImage.fileName));
  instance_negative.setImage(formatUrl(negativeImage.bucketName, negativeImage.fileName));
  selectedList.value.push(nextfileId);
  selectedIndex.value = index;
  resizeReferenceCanvases();
...
const state: BasicTableProps = reactive<BasicTableProps>({
  ...
  queryForm: {
    baseDatasetId: '',
    bucketName: 'dataset/triplets_shard_0000/base/images/dataset/anchor',
    tagId: '',
    taskId: '',
    fileName: '',
    isLabeled: '',
...
const openCanvas = async (param: any) => {
  ...
  try {
    ...
    if (res && res.data) {
      ...
      state.queryForm.taskId = "1954721123684265988";
...
const positiveRef = ref<HTMLCanvasElement | null>(null);
const negativeRef = ref<HTMLCanvasElement | null>(null);
// 上半部分 Positive 区域状态（正常/有误）
const positiveStatus = ref(false);
const negativeStatus = ref(false);
const mode = ref(0);
...
const getTaskList = async () => {
  ...
  const res = await fetchTaskList({ baseDatasetId: taskForm.baseDatasetId, labelType: taskForm.labelType, subLabelType: "0" });
  if (res && res.data) {
    ...
    imageList.value.forEach((image) => {
      if (selectedList.value.some(item => item === image.fileId)) {
        smartForm.data_list.push({
          fileId: image.fileId,
          imgPath: formatUrl(image.bucketName, image.fileName, true),
          height: image.height,
          width: image.width
        })
      }
...
const resizeReferenceCanvases = () => {
  const resizeSingle = (
    canvasEl: HTMLCanvasElement | null,
    instanceRef: CanvasSelect,
    images: Ref<imageItem[]>
  ) => {
    if (!canvasEl) return;
    const wrapper = canvasEl.parentElement as HTMLElement | null;
    if (!wrapper) return;
    const { clientWidth, clientHeight } = wrapper;
    if (!clientWidth || !clientHeight) return;
    if (canvasEl.width !== clientWidth) {
      canvasEl.width = clientWidth;
    }
    if (canvasEl.height !== clientHeight) {
      canvasEl.height = clientHeight;
    }
    if (selectedIndex.value >= 0 && images.value[selectedIndex.value]) {
      const currentImage = images.value[selectedIndex.value];
      instanceRef.resize(
        clientWidth,
        clientHeight,
        1,
        formatUrl(currentImage.bucketName, currentImage.fileName)
      );
    }
  };

  resizeSingle(positiveRef.value, instance_positive, imagePositiveList);
  resizeSingle(negativeRef.value, instance_negative, imageNegativeList);
};
...
const handleResize = () => {
  ...
  if (selectedIndex.value >= 0 && imageList.value.length !== 0) {
    instance.resize(width, height, 1, formatUrl(imageList.value[selectedIndex.value].bucketName, imageList.value[selectedIndex.value].fileName));
    instance_bgimg.resize(width, height, 1, formatUrl(imageList.value[selectedIndex.value].bucketName, imageList.value[selectedIndex.value].fileName));
    instance_temp.resize(width, height, 1, formatUrl(imageList.value[selectedIndex.value].bucketName, imageList.value[selectedIndex.value].fileName));
...
onMounted(async () => {
  // 判断taskId是否存在
  // if ("1954721123684265988" && typeof "1954721123684265988" === 'string') {
  try {
    handelLoading.value = true;
    const res = await getTaskObj("1954721123684265988");
    state.queryForm.baseDatasetId = res.data.baseDatasetId;
    state.queryForm.taskId = "1954721123684265988";
  ...
  instance_positive = new CanvasSelect('.positive-container');
  instance_negative = new CanvasSelect('.negative-container');
  instance_bgimg = new CanvasSelect('.bgimage');
  instance_temp = new CanvasSelect('.tempcanvas');
  instance.setImage('');
  instance_positive.setImage('');
  instance_negative.setImage('');
  instance.readonly = true; //初始为查看模式
  instance_positive.readonly = true; //初始为查看模式
  instance_negative.readonly = true; //初始为查看模式
  ...
  const bucketNameParam = 'dataset/' + "triplets_shard_0000" + '/base/images/dataset';
  ...
  nextTick(() => resizeReferenceCanvases());
  ...
  instance_positive.on('load', (src: string) => {
    console.log('instance_positive图片加载完成Image*******************', src);
    // instance.setImage(formatUrl(currentbucketName.value, currentfileName.value), 0.5);
  });
  instance.on("add", (info: any) => {
    ...
    if (tabBarAreaRef.value.lockedItem == '') {
      // 更新菜单的位置并显示菜单
      menuPosition.value = { x: mousePosition.value.x, y: mousePosition.value.y };
      menuVisible.value = true;
      menuUuid.value = instance.activeShape.uuid;
    }
  });
  ...
  instance_positive.on("updated", (result: any) => {
    console.log("****", result);
    console.log("****", instance_positive);
  });
...
onBeforeUnmount(() => {
  ...
  instance_positive.destroy();
  instance_negative.destroy();
});
// 注销时打印
onUnmounted(() => {
...
watch(() => state.dataList, async (newVal) => {
  ...
  if (state.dataList && state.dataList.length > 0) {
    if (!imageList.value.length) {
      ...
      // imageViewList.value.push(...(state.dataList ?? []).map((item: imageItem) => formatUrl(item.bucketName, item.fileName)));
      // 在 DOM 更新完成后执行下面的逻辑
      // try {
...
watch(() => state.pagination?.current, () => {
  ...
  getLabeledTotal({ taskId: "1954721123684265988" }).then((res) => {
    tagedNumber.value = res.data.total;
  }).catch((err) => {
    useMessage().error(err.msg);
  });
...
/* Overrides for left-top layout and canvases to ensure 50/50 split and containment */
.left-top {
  display: flex;
  flex-direction: row;
  justify-content: flex-start;
...
/* Ensure canvases fill their container but never overflow */
.left-top .positive-container,
.left-top .negative-container {
  width: 100% !important;
...
.left-top .left-top-right>div[style*="position: relative"] {
  min-height: 0;
}

.left-top .bgimage,
.left-top .canvas-container,
.left-top .tempcanvas {
  top: 0;
  left: 0;
  width: 100% !important;
...
/* Overlays for the upper (Positive) canvas */
.canvas-wrapper {
  position: relative;
}
