<template>
  <el-dialog
    ...
    :title="form.jobId ? $t('common.editBtn') : $t('common.addBtn')"
    draggable
  >
    <el-form
      ref="dataFormRef"
    ...
    >
      <el-row :gutter="20">
        ...
        <el-col
          :span="12"
          class="mb20"
          v-if="['3', '4'].includes(form.jobType)"
        >
          <el-form-item :label="t('job.executePath')" prop="executePath">
            <el-input
              v-model="form.executePath"
              :placeholder="t('job.inputexecutePathTip')"
            />
          </el-form-item>
        </el-col>
        ...
        <el-col
          :span="12"
          class="mb20"
          v-if="['1', '2'].includes(form.jobType)"
        >
          <el-form-item :label="t('job.className')" prop="className">
            <el-input
              v-model="form.className"
              :placeholder="t('job.inputclassNameTip')"
            />
          </el-form-item>
        </el-col>
        ...
        >
          <el-form-item :label="t('job.methodName')" prop="methodName">
            <el-input
              ...
              :placeholder="t('job.inputmethodNameTip')"
            />
          </el-form-item>
        </el-col>
        ...
        <el-col :span="12" class="mb20">
          <el-form-item
            :label="t('job.methodParamsValue')"
            prop="methodParamsValue"
          >
            <el-input
              v-model="form.methodParamsValue"
              :placeholder="t('job.inputmethodParamsValueTip')"
            />
          </el-form-item>
        </el-col>
        ...
        <el-col :span="12" class="mb20">
          <el-form-item :label="t('job.misfirePolicy')" prop="misfirePolicy">
            <el-select
              ...
              :placeholder="t('job.inputmisfirePolicyTip')"
            >
              <el-option
                v-for="(item, index) in misfire_policy"
                :key="index"
                :label="item.label"
                :value="item.value"
              ></el-option>
        ...
        <el-col :span="24" class="mb20">
          <el-form-item :label="t('job.remark')" prop="remark">
            <el-input
              v-model="form.remark"
              :placeholder="t('job.inputremarkTip')"
              type="textarea"
            />
          </el-form-item>
        </el-col>
    ...
    <template #footer>
      <span class="dialog-footer">
        <el-button formDialogRef @click="visible = false">{{
...
const Crontab = defineAsyncComponent(() =>
  import("/@/components/Crontab/index.vue")
);
...
// 定义字典
const { misfire_policy, job_type } = useDict(
  "job_status",
  "job_execute_status",
  "misfire_policy",
  "job_type"
...
const onSubmit = async () => {
  ...
  try {
    ...
    useMessage().success(
      t(form.jobId ? "common.editSuccessText" : "common.addSuccessText")
    );
    visible.value = false;
    emit("refresh");
  } catch (err) {
