  ...
  <el-card style="height: 100%;">
    <div style="display: flex; justify-content: space-between;">
      <div style="display: flex;">
        ...
        <div class="info">
          <span style="font-weight: 600; margin: 2px; font-size: 18px;">{{
          userData.name
        }}</span>
...
const initUserInfo = async (userId: any): Promise<void> => {
  try {
    ...
    userData.value.postName =
      res.data?.postList?.map((item: any) => item.postName).join(",") || ""; // 将 postList 中的 postName 合并成字符串并保存到 userData 变量中
    // 文件上传增加后端前缀
    if (userData.value.avatar != null && !(userData.value.avatar.indexOf("http") !== -1)) {
