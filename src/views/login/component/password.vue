<template>
  <el-form size="large" class="login-content-form" ref="loginFormRef" :rules="loginRules" :model="state.ruleForm"
    ...
    <el-form-item class="login-animation1" prop="username">
      <el-input text :placeholder="$t('password.accountPlaceholder4')" v-model="state.ruleForm.username" clearable
        autocomplete="off">
        <!-- <template #prefix>
    ...
    <el-form-item>
      <span class="login-accept">{{ $t('password.loginReadAccept') }}</span>
...
const loginRules = reactive({
  username: [{ required: true, trigger: 'blur', message: t('password.accountPlaceholder4') }], // 用户名校验规则
